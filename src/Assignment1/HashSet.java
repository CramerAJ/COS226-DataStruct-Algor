...
//left all comment in /* */ the same, used them as a helper to understand the work below.


import java.io.Serializable;
import java.util.AbstractCollection;            	 //  imported AbstractCollection
import java.util.Collection;
import java.util.ConcurrentModificationException;    //  imported ConcurrentModification
import java.util.Iterator;                     		 //  imported Iterator 
...
import java.util.Set;
	...
	public class HashSet<AnyType> extends AbstractCollection<AnyType> implements Set<AnyType>
	{
	    ...
	    public boolean add( AnyType x )
	    {
	        ...
	        HashEntry list = array[currentPos];
	        
	        while (array[currentPos].catdog != null){

	        	list.catdog = list; //  unclear a bit on what I was trying to do here, assumed adding the two
	        					   //  
	        }	       
	        												// lengtharray = list.catdog; thought this would work    
	        HashEntry lengtharray1 = new HashEntry(x);
			//new HashEntry for trying to decrease array size
	        list.catdog = lengtharray1;
			//trying to decrease the size of the array by 1 to note that the spot is now taken up by x
	        
	        lengtharray = array.length-1;      //reduces array size 
	        
	        while (lengtharray1 != null){		//while loop to reduce the size of the array to fill up space
	        	lengtharray=(array.length - 1) ;					
	        }
	    ...
	    public boolean remove( Object x )
	    {
	        ...
	        //remove loop on the HashEntry catdog
	        while(array[currentPos].catdog !=null){
	        	
	        	if(array[currentPos].catdog.equals(x)){
	        		array[currentPos].isActive = false;		
	        	}
	        	else{
	        		array[currentPos] = array[currentPos].catdog;
	        	}
	        		
	        	//will remove the currentPos or x selected from the table
	        	//array[currentPos].catdog.remove(currentPos);
	    ...
	    private static class HashEntry implements Serializable
	    {
	        ...
	        public HashEntry catdog;  //  pointing to another HashEntry, added a HashEntry
	    ...
	    {
	        //int offset = 1;                          //  quadratic searching
	        int currentPos = ( x == null ) ? 0 : Math.abs( x.hashCode( ) % array.length );
	        //  pushing it all to the top of the table
	        //HashEntry currentPos = array [currentPos]; //  calling the new HashEntry to the currentPos array
	        int starting = 0;  
	        ...
	        
	        {
	            ...
	            else if(starting == 0) { // checks to see the if the current position is the starting position of the Array
	            	currentPos=starting;
	            	
	            }
	            else {					 // if it isn't 0 it is then added to table size and increases in size
	            	starting++;
	            	currentPos = starting;
	            }
	            
	           // currentPos += offset;                 removed this since its quadratic
	           // offset += 2;							removed since its quadratic
